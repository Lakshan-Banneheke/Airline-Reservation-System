...
class Flight {
    ...
    static async getAllFlights() {
        const query = 'SELECT schedule_id,departure_date,departure_time_utc,arrival_date,arrival_time_utc,origin,destination,price FROM flight_schedule INNER JOIN route USING(route_id) INNER JOIN seat_price USING(route_id) WHERE departure_date>NOW() AND traveler_class_id=3  ORDER BY departure_date LIMIT 6;';
